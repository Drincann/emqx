...
handle_call({add_route, Topic, SessionId}, _From, State) ->
    Ok = do_add_route(Topic, SessionId),
    {reply, Ok, State};

handle_call({delete_route, Topic, SessionId}, _From, State) ->
    Ok = do_delete_route(Topic, SessionId),
    {reply, Ok, State};
